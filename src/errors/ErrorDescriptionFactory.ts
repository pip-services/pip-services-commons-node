...
export class ErrorDescriptionFactory {
	...
	/**
	 * @param error  	'error' can be an instance of ApplicationException (in which case the ErrorDescription
     * is generated based on the data in the ApplicationException), or simply contain error.name (its type), 
     * error.message (error.toString(), if not available), and error.stack. In the second case, the ErrorDescription's 
